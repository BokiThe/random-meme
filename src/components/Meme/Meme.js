...
const Meme = () => {
  ...
  const handleSelect = (e) => {
    e.preventDefault();
    memes.findIndex((curr, i) =>
      curr.name === e.target.value ? setMemeIndex(i) : false
    );
  };
  ...
  return memes.length ? (
    <div className={styles.container}>
      <h4>
        Fill the descriptions to make your own meme and click{" "}
        <span>generate</span> or just click <span>skip</span> to find meme you
        want{" "}
      </h4>
      ...
      <select
        name="Select meme"
        id="selectMeme"
        value={memes[memeIndex].name}
        className={styles.caption}
        onChange={handleSelect}
      >
        {memes.map((meme, i) => (
          <option key={i} id={i} value={meme.name}>
            {meme.name}
          </option>
        ))}
      </select>
